...
# --- 1. DATA IDENTITAS TOTAL (SUDAH LENGKAP) ---
...
# DATA LOGIN MT5 (TAMBAHAN FINAL)
MT5_ACCOUNT_ID = "211174033"
MT5_SERVER = "ValetaxIntl-Live2"
MT5_PASSWORD = "GANTI_DENGAN_PASSWORD_MU" # <--- Isi password Valetax di sini!
...
async def jalankan_trading():
    ...
    try:
        ...
        print(f"✅ Berhasil Login ke {MT5_SERVER}")
    except Exception as e:
        print(f"❌ Gagal Login: {e}")

# --- 4. TELEGRAM HANDLER ---
@bot.message_handler(commands=['start', 'cek'])
def send_report(message):
    teks = (
        "⚔️ **JARVIS SUDAH SIAP TEMPUR** ⚔️\n"
        f"👤 Akun: {MT5_ACCOUNT_ID}\n"
        f"🌐 Server: {MT5_SERVER}\n"
        "🤖 Otak: Gemini AI Aktif\n"
        "🎯 Target: 1200 cent\n"
        "---------------------------\n"
        "✅ Status: Menunggu Password untuk Transaksi!"
    )
    bot.reply_to(message, teks, parse_mode="Markdown")

@bot.message_handler(func=lambda message: True)
def chat_ai(message):
    prompt = f"Kamu Jarvis, asisten Elino. Jawab cerdas: {message.text}"
    try:
        response = model.generate_content(prompt)
        bot.reply_to(message, response.text)
    except:
        bot.reply_to(message, "Lagi fokus trading, Bos!")
