...
def send_report(message):
    # Menggunakan loop untuk menjalankan fungsi async di dalam handler sync
...
def chat_ai(message):
    try:
        # Perbaikan prompt agar Gemini lebih responsif
        prompt = f"Kamu adalah Jarvis, asisten trading setia Bos Elino. Jawab dengan cerdas dan singkat: {message.text}"
        response = model.generate_content(prompt)
        bot.reply_to(message, response.text)
    except Exception as e:
        print(f"AI Error: {e}")
        bot.reply_to(message, "Maaf Bos, otak AI saya sedang refresh. Coba lagi sedetik lagi!")
