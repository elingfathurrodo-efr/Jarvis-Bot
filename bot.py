...
# --- 4. ENGINE HUNTER (AUTO ENTRY & TARGET 1200) ---
async def hunter_engine():
    conn = await connect_to_mt5()
    if not conn: return
    print(f"🚀 JARVIS: Berhasil Login Akun {MT5_ACCOUNT}. Mulai Berburu!")
    ...
    while True:
        try:
            ...
            # Berhenti jika target 1200 cent tercapai
            if balance >= 1200:
                print("🎯 Target Tercapai! Jarvis Standby.")
                break
            
            # LOGIKA AUTO ENTRY:
            # Jika tidak ada posisi terbuka, Jarvis akan Entry BUY Gold
            positions = await conn.get_positions()
            if len(positions) == 0:
                print("⚔️ Jarvis: Deteksi peluang, melakukan Entry BUY XAUUSD...")
                # Lot 0.01 dengan TP di saldo 1200
                await conn.create_market_order('XAUUSD', 'BUY', 0.01)
            
            await asyncio.sleep(60) # Cek setiap menit
        except Exception as e:
            print(f"⚠️ Reconnecting Engine... {e}")
            conn = await connect_to_mt5()
            await asyncio.sleep(10)

# --- 5. TELEGRAM HANDLER (OBROLAN AI) ---
@bot.message_handler(func=lambda message: True)
def main_handler(message):
    txt = message.text.lower()
    
    # Perintah Status Saldo Rill
    if any(k in txt for k in ["saldo", "cek", "berapa"]):
        ...
        if conn:
            ...
            teks = (
                ...
                f"💰 Saldo Rill: {info['balance']} cent\n"
                f"🎯 Target: 1200 cent\n"
                f"📈 Status: {'Berburu' if info['balance'] < 1200 else 'Target Tercapai'}"
            )
            bot.reply_to(message, teks, parse_mode="Markdown")
        else:
            bot.reply_to(message, "Gagal konek ke MT5, Bos!")

    # Tanya Jawab Pintar Gemini
    else:
        try:
            prompt = f"Kamu Jarvis, asisten AI Elino. Jawab dengan gaya asisten cerdas: {message.text}"
            res = model.generate_content(prompt)
            bot.reply_to(message, res.text)
        except:
            bot.reply_to(message, "Otak saya lagi panas Bos, tanya lagi bentar lagi ya!")

if __name__ == "__main__":
    # Menjalankan mesin hunter (Auto Entry) di background
    threading.Thread(target=lambda: asyncio.run(hunter_engine())).start()
    print("🤖 Jarvis Online & Menjaga Akun...")
