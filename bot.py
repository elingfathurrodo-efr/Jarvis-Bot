...
# --- 1. DATA IDENTITAS TOTAL (SUDAH TERPASANG) ---
...
# DATA LOGIN MT5 (SUDAH TERPASANG)
...
# --- 3. FUNGSI TRANSAKSI OTOMATIS ---
async def jalankan_trading():
    ...
    try:
        ...
        print(f"✅ Login Berhasil ke {MT5_SERVER}")
        # Jarvis mulai eksekusi transaksi otomatis di market
    except Exception as e:
        print(f"❌ Gagal Login: {e}")

# --- 4. TELEGRAM HANDLER ---
@bot.message_handler(commands=['start', 'cek', 'profit'])
def send_report(message):
    teks = (
        "⚔️ **JARVIS MODE TEMPUR TOTAL (ELINO)** ⚔️\n"
        "---------------------------\n"
        f"👤 Akun: {MT5_ACCOUNT_ID}\n"
        f"🌐 Server: {MT5_SERVER}\n"
        "🤖 Otak AI: Gemini Aktif\n"
        "💰 Status: Terhubung & Siap Profit\n"
        "🎯 Target: 1200 cent\n"
        "---------------------------\n"
        "✅ JARVIS: Sedang berburu Gold secara otomatis!"
    )
    bot.reply_to(message, teks, parse_mode="Markdown")

@bot.message_handler(func=lambda message: True)
def chat_ai(message):
    prompt = f"Kamu adalah Jarvis, asisten trading setia Bos Elino. Jawab bos dengan loyal dan cerdas: {message.text}"
    try:
        response = model.generate_content(prompt)
        bot.reply_to(message, response.text)
    except:
        bot.reply_to(message, "Lagi fokus jagain profit, Bos!")

if __name__ == "__main__":
    # Menjalankan mesin trading dan Telegram sekaligus
    threading.Thread(target=lambda: asyncio.run(jalankan_trading())).start()
    bot.polling()
